...
int strpreproc(char *s, unsigned char *mlcom)
{
	// If multiline remains
	if (*mlcom && (strstr(s, ">_") == NULL))
		s[0] = '\0';
	
	// Check for multiline comment
	else 
	while (((strstr(s, "_<") != NULL) && (strnchrcount(s, '"', strstr(s, "_<") - s) % 2 == 0) && (strnchrcount(s, '\'', strstr(s, "_<") - s) % 2 == 0) && (!*mlcom))
	||     ((strchr(s, '#')  != NULL) && (strnchrcount(s, '"', strchr(s,  '#') - s) % 2 == 0) && (strnchrcount(s, '\'', strchr(s,  '#') - s) % 2 == 0) && (!*mlcom))
	||     ((strstr(s, ">_") != NULL) && (*mlcom)))
	{
